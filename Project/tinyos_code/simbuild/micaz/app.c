...
bool connReceived[8] = { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE };
bool connAckReceived[8] = { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE };
int8_t topicSubscriptions[8] = { 0, 1, 1, 0, 2, 1, 2, 0 };
bool subReceived[8] = { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE };
bool subAckReceived[8] = { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE };
...
queue_t *Project1C$queue[1000];




static inline void Project1C$queueInit(queue_t *q);
...
static inline bool Project1C$enqueue(queue_t *q, msg_t *msg);
#line 74
static inline bool Project1C$dequeue(queue_t *q);
...
static inline bool Project1C$peek(queue_t *q, msg_t *msg);
#line 120
static uint8_t Project1C$generateRandomValue(uint8_t topic);
#line 155
...
#line 75
  if (__nesc_ntoh_int16(q->count.nxdata) <= 0) {
      return FALSE;
    }
  __nesc_hton_int16(q->front.nxdata, (__nesc_ntoh_int16(q->front.nxdata) + 1) % 10);
  (__nesc_temp44 = q->count.nxdata, __nesc_hton_int16(__nesc_temp44, (__nesc_temp45 = __nesc_ntoh_int16(__nesc_temp44)) - 1), __nesc_temp45);
  return TRUE;
}
...
#line 84
{
  ...
  memcpy(msg, &q->buffer[__nesc_ntoh_int16(q->front.nxdata)], sizeof(msg_t ));
...
#line 63
  if (__nesc_ntoh_int16(q->count.nxdata) >= 10) {
      return FALSE;
    }
  else 
#line 65
    {
      __nesc_hton_int16(q->rear.nxdata, (__nesc_ntoh_int16(q->rear.nxdata) + 1) % 10);
      memcpy(&q->buffer[__nesc_ntoh_int16(q->rear.nxdata)], msg, sizeof(msg_t ));
      (__nesc_temp42 = q->count.nxdata, __nesc_hton_int16(__nesc_temp42, (__nesc_temp43 = __nesc_ntoh_int16(__nesc_temp42)) + 1), __nesc_temp43);
      return TRUE;
    }
}
      ...
      if (TOS_NODE_ID != 1) 
        {
          ...
          if (__nesc_ntoh_uint8(msg->type.nxdata) == 1 && !connAckReceived[TOS_NODE_ID - 2]) 
            {

              msg_t *msg = (msg_t *)Project1C$Packet$getPayload(&Project1C$packet[sim_node()], sizeof(msg_t ));

#line 224
              sim_log_debug(157U, "radio_rec", "NODE %d: CONNACK received. Connection successful.\n", TOS_NODE_ID - 1);
              connAckReceived[TOS_NODE_ID - 2] = TRUE;




              __nesc_hton_uint8(msg->topic.nxdata, topicSubscriptions[TOS_NODE_ID - 2]);

              __nesc_hton_uint8(msg->type.nxdata, 2);
              __nesc_hton_uint8(msg->destination.nxdata, 1);
              __nesc_hton_uint8(msg->sender.nxdata, TOS_NODE_ID);

              switch (__nesc_ntoh_uint8(msg->topic.nxdata)) {
                  case 0: 
                    sim_log_debug(158U, "radio_send", "NODE %d: Sending a SUB to TEMPERATURE topic...\n", TOS_NODE_ID - 1);
                  break;
                  case 1: 
                    sim_log_debug(159U, "radio_send", "NODE %d: Sending a SUB to HUMIDITY topic...\n", TOS_NODE_ID - 1);
                  break;
                  case 2: 
                    sim_log_debug(160U, "radio_send", "NODE %d: Sending a SUB to LUMINOSITY topic...\n", TOS_NODE_ID - 1);
                  break;
                  default: 
                    break;
                }

              Project1C$AMSend$send(__nesc_ntoh_uint8(msg->destination.nxdata), &Project1C$packet[sim_node()], sizeof(msg_t ));
            }



          if (__nesc_ntoh_uint8(msg->type.nxdata) == 3 && !subAckReceived[TOS_NODE_ID - 2]) 
            {
              switch (__nesc_ntoh_uint8(msg->topic.nxdata)) {
                  case 0: 
                    sim_log_debug(161U, "radio_rec", "NODE %d: SUBACK received. I am subscribed to TEMPERATURE topic.\n", TOS_NODE_ID - 1);
                  break;
                  case 1: 
                    sim_log_debug(162U, "radio_rec", "NODE %d: SUBACK received. I am subscribed to HUMIDITY topic.\n", TOS_NODE_ID - 1);
                  break;
                  case 2: 
                    sim_log_debug(163U, "radio_rec", "NODE %d: SUBACK received. I am subscribed to LUMINOSITY topic.\n", TOS_NODE_ID - 1);
                  break;
                  default: 
                    break;
                }
          ...
          if (connAckReceived[TOS_NODE_ID - 2] && subAckReceived[TOS_NODE_ID - 2]) 
            {
              ...
              switch (__nesc_ntoh_uint8(msg->topic.nxdata)) {
                  case 0: 
                    __nesc_hton_uint8(msg->data.nxdata, Project1C$generateRandomValue(0));
                  sim_log_debug(164U, "radio_send", "NODE %d: Sending a PUB to TEMPERATURE topic with payload: %d°C\n", TOS_NODE_ID - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;

                  case 1: 
                    __nesc_hton_uint8(msg->data.nxdata, Project1C$generateRandomValue(1));
                  sim_log_debug(165U, "radio_send", "NODE %d: Sending a PUB to HUMIDITY topic with payload: %d%\n", TOS_NODE_ID - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;

                  case 2: 
                    __nesc_hton_uint8(msg->data.nxdata, Project1C$generateRandomValue(2));
                  sim_log_debug(166U, "radio_send", "NODE %d: Sending a PUB to LUMINOSITY topic with payload: %d lux\n", TOS_NODE_ID - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;
      ...
      if (TOS_NODE_ID == 1) 
        {
          ...
          if (__nesc_ntoh_uint8(msg->type.nxdata) == 0) 
            {
              sim_log_debug(167U, "radio_rec", "PAN Coordinator: received CONN from node: %d\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1);
              connReceived[__nesc_ntoh_uint8(msg->sender.nxdata) - 2] = TRUE;

              sim_log_debug(168U, "radio_send", "PAN Coordinator: sending CONNACK to node: %d...\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1);

              __nesc_hton_uint8(msg->type.nxdata, 1);
              __nesc_hton_uint8(msg->destination.nxdata, __nesc_ntoh_uint8(msg->sender.nxdata));
              __nesc_hton_uint8(msg->sender.nxdata, 1);


              Project1C$AMSend$send(__nesc_ntoh_uint8(msg->destination.nxdata), bufPtr, sizeof(msg_t ));
            }
          ...
          if (__nesc_ntoh_uint8(msg->type.nxdata) == 2) 
            {
              sim_log_debug(169U, "radio_rec", "PAN Coordinator: received SUB from node: %d\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1);
              subReceived[__nesc_ntoh_uint8(msg->sender.nxdata) - 2] = TRUE;

              sim_log_debug(170U, "radio_send", "PAN Coordinator: sending SUBACK to node: %d...\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1);
          ...
          if (__nesc_ntoh_uint8(msg->type.nxdata) == 4) 
            {
              switch (__nesc_ntoh_uint8(msg->topic.nxdata)) {
                  ...
                  case 0: 
                    sim_log_debug(171U, "radio_rec", "PAN Coordinator: PUB received from node %d to topic TEMPERATURE with payload: %d°C\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;

                  case 1: 
                    sim_log_debug(172U, "radio_rec", "PAN Coordinator: PUB received from node %d to topic HUMIDITY with payload: %d%\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;

                  case 2: 
                    sim_log_debug(173U, "radio_rec", "PAN Coordinator: PUB received from node %d to topic LUMINOSITY with payload: %d lux\n", __nesc_ntoh_uint8(msg->sender.nxdata) - 1, __nesc_ntoh_uint8(msg->data.nxdata));
                  break;

                  default: 
                    break;
                }
              Project1C$enqueue(Project1C$queue[sim_node()], msg);
            }



          if (Project1C$peek(Project1C$queue[sim_node()], msg)) {
              ...
              if (__nesc_ntoh_uint8(msg->type.nxdata) == 4) 
                {
                  ...
                  for (Project1C$i[sim_node()] = 0; Project1C$i[sim_node()] < 8; Project1C$i[sim_node()]++) 
                    {
                      if (topicSubscriptions[Project1C$i[sim_node()]] == __nesc_ntoh_uint8(msg->topic.nxdata) && subAckReceived[Project1C$i[sim_node()]] && Project1C$i[sim_node()] != __nesc_ntoh_uint8(msg->sender.nxdata) - 2) 
                        {
                          sim_log_debug(174U, "radio_rec", "PAN Coordinator: Forwarding PUB to the subscribed node: %d...\n", Project1C$i[sim_node()] + 1);
                          Project1C$AMSend$send(Project1C$i[sim_node()] + 1, bufPtr, sizeof(msg_t ));
                        }
                    }

                  sim_log_debug(175U, "radio_rec", "PAN Coordinator: Sending PUB messages to Node-RED. Contains: topic: %d, data: %d\n", __nesc_ntoh_uint8(msg->topic.nxdata), __nesc_ntoh_uint8(msg->data.nxdata));
                  ...
                  if (Project1C$sockfd[sim_node()] == -1) 
                    {
                      sim_log_debug(176U, "error", "PAN Coordinator: Socket creation failed!\n");
                      return bufPtr;
                    }
                  ...
                  Project1C$servaddr[sim_node()].sin_port = htons(1048);


                  if (connect(Project1C$sockfd[sim_node()], (struct sockaddr *)&Project1C$servaddr[sim_node()], sizeof Project1C$servaddr[sim_node()]) != 0) 
                    {
                      sim_log_debug(177U, "error", "PAN Coordinator: Connection with the server failed!\n");
                  ...
                  if (send(Project1C$sockfd[sim_node()], msg, sizeof(msg_t ), 0) == -1) 
                    {
                      sim_log_debug(178U, "error", "PAN Coordinator: Failed to send message!\n");
                      return bufPtr;
                    }


                  Project1C$dequeue(Project1C$queue[sim_node()]);
...
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Receive$receive(am_id_t arg_0xb76b1ac8, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
  switch (arg_0xb76b1ac8) {
#line 78
    case 10:
#line 78
...
#line 78
    default:
#line 78
      __nesc_result = TossimActiveMessageC$Receive$default$receive(arg_0xb76b1ac8, msg, payload, len);
#line 78
      break;
#line 78
    }
...
#line 162
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb745c498, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb745c498, addr, msg, len);
#line 80
...
#line 75
# 110 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static void TossimActiveMessageC$AMSend$sendDone(am_id_t arg_0xb76b1108, message_t * msg, error_t error){
#line 110
  /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$sendDone(arg_0xb76b1108, msg, error);
#line 110
}
#line 110
...
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Snoop$receive(am_id_t arg_0xb76af178, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC$Snoop$default$receive(arg_0xb76af178, msg, payload, len);
#line 78
...
static inline void Project1C$AMControl$startDone(error_t err)
{
  if (err == SUCCESS) {
      ...
      if (TOS_NODE_ID == 1) {
          sim_log_debug(152U, "radio", "PAN Coordinator: Radio ON.\n");
        }
      else {
          sim_log_debug(153U, "radio", "NODE %d: Radio ON.\n", TOS_NODE_ID - 1);
        }
      Project1C$MilliTimer$startPeriodic(250);
    }
  else {
      sim_log_error(154U, "radio", "Radio start failed, retrying...\n");
...
static inline void Project1C$AMControl$stopDone(error_t err)
{
  sim_log_debug(155U, "boot", "Radio stopped.\n");
}
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb745fa98, message_t * msg, error_t error){
#line 100
  switch (arg_0xb745fa98) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb745fa98, msg, error);
#line 100
      break;
#line 100
    }
      ...
      if (TOS_NODE_ID != 1 && !connReceived[TOS_NODE_ID - 2]) 
        {
          ...
          Project1C$AMSend$send(__nesc_ntoh_uint8(msg->destination.nxdata), &Project1C$packet[sim_node()], sizeof(msg_t ));
          sim_log_debug(156U, "radio_send", "NODE %d: Sending CONN message to the PAN Coordinator...\n", TOS_NODE_ID - 1);
        }
    }
}
...
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0xb72dd3a8){
#line 83
  switch (arg_0xb72dd3a8) {
#line 83
    case 0U:
#line 83
...
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0xb72dd3a8);
#line 83
      break;
#line 83
    }
...
static inline void Project1C$queueInit(queue_t *q)
{
  __nesc_hton_int16(q->front.nxdata, 0);
  __nesc_hton_int16(q->rear.nxdata, -1);
  __nesc_hton_int16(q->count.nxdata, 0);
}

#line 155
static inline void Project1C$Boot$booted(void )
{
  Project1C$queue[sim_node()] = (queue_t *)malloc(sizeof(queue_t ));
  Project1C$queueInit(Project1C$queue[sim_node()]);
  ...
  if ((!connReceived[TOS_NODE_ID - 2] || !connAckReceived[TOS_NODE_ID - 2]) && TOS_NODE_ID != 1) 
    {
      ...
      sim_log_debug(179U, "radio_send", "NODE %d: CONN Packet lost. Sending again a CONN to PAN...\n", TOS_NODE_ID - 1);
      __nesc_hton_uint8(msg->type.nxdata, 0);
      __nesc_hton_uint8(msg->destination.nxdata, 1);
      __nesc_hton_uint8(msg->sender.nxdata, TOS_NODE_ID);
      Project1C$AMSend$send(__nesc_ntoh_uint8(msg->destination.nxdata), &Project1C$packet[sim_node()], sizeof(msg_t ));
  ...
  if ((!subReceived[TOS_NODE_ID - 2] || !subAckReceived[TOS_NODE_ID - 2]) && TOS_NODE_ID != 1 && connAckReceived[TOS_NODE_ID - 2]) 
    {
      ...
      switch (topicSubscriptions[TOS_NODE_ID - 2]) {
          case 0: 
            sim_log_debug(180U, "radio_send", "NODE %d: Packet lost. Sending again a SUB to PAN for TEMPERATURE topic\n", TOS_NODE_ID - 1);
          break;

          case 1: 
            sim_log_debug(181U, "radio_send", "NODE %d: Packet lost. Sending again a SUB to PAN for HUMIDITY topic.\n", TOS_NODE_ID - 1);
          break;

          case 2: 
            sim_log_debug(182U, "radio_send", "NODE %d: Packet lost. Sending again a SUB to PAN for LUMINOSITY topic.\n", TOS_NODE_ID - 1);
          break;
...
#line 120
{
  ...
  switch (topic) {
      ...
      case 2: 
        ...
        value = (uint8_t )(Project1C$Random$rand16() % 255);
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  if (!strcmp(varname, "Project1C$queue"))
  {
    *addr = (uintptr_t)&Project1C$queue[__nesc_mote];
    *size = sizeof(Project1C$queue[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  memset((void *)&Project1C$queue[__nesc_mote], 0, sizeof Project1C$queue[__nesc_mote]);
